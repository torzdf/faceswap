...
class ExtractPlugin(abc.ABC):
    ...
    def __init__(self,
        ...
        """int : The size of the plugin's input in pixels"""
        self.name = self.__class__.__name__
        """str : The name of the plugin. Derived from the module name"""
        self.batch_size = batch_size
        """int : The maximum batch size that this plugin's 'process' method will receive"""
        self.is_rgb = is_rgb
        """bool : ``True`` if the plugin expects RGB images. ``False`` for BGR"""
        self.dtype = dtype
        """str : The datatype that the plugin expects images at"""
        self.scale = scale
        """tuple[int, int] : The numeric range that the plugin expects images to be in"""
        self.device = self._get_device(force_cpu)
        """:class:`torch.device` : The selected device to run torch ops on"""
    ...
    def pre_process(self, batch: np.ndarray) -> np.ndarray:
        ...
        -------
        :class:`numpy.ndarray`
            For align plugins, this should be an adjustment of the detected face's bounding box to
            cut a square out of the original image for feeding the model. Out of bounds values are
            allowed, as these will be handled. This bounding box will be used to prepare the image
            at the correct size for feeding the model.

            For all other plugins, any pre-processing (eg normalization) should be applied ready
    ...
    def process(self, batch: np.ndarray) -> np.ndarray:
        ...
        -------
        :class:`numpy.ndarray`
            This can return any numpy array, but it must be a numpy array
        """
    ...
    def post_process(self, batch: np.ndarray) -> npt.NDArray[np.float32]:
        ...
        :class:`numpy.ndarray`
            ...
            detected faces scaled to model input size as float32. N is the batch size, M is the
            number of detections per batch

            For align plugins this must be an (N, 68, 2) float32 array for each (x, y) landmark
            point for each face in the batch. co-ordinates should be normalized to 0.0 to 1.0 range

            For mask plugins this must be an (N, size, size) float32 image in range 0. - 1.0 for
            each face in the batch
...
class FacePlugin(ExtractPlugin):
    ...
    def __init__(self,
        ...
        """Literal["legacy", "face", "head"] : The aligned centering of the image patch to feed the
        model"""
